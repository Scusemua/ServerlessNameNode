package com.gmail.benrcarver.serverlessnamenode.hops.resolvingcache;
...
public abstract class Cache {
        ...
        if(isStarted){
            ...
            if(TransactionsStats.getInstance().isEnabled()){
                TransactionsStats.getInstance().pushResolvingCacheStats(
                        new TransactionsStats.ResolvingCacheStat(
                                TransactionsStats.ResolvingCacheStat.Op.SET, 0,
    ...
    public final void set(final INode inode){
        if(isStarted){
            ...
            if(TransactionsStats.getInstance().isEnabled()){
                TransactionsStats.getInstance().pushResolvingCacheStats(
                        new TransactionsStats.ResolvingCacheStat(
                                TransactionsStats.ResolvingCacheStat.Op.SET, 0,1));
            }
        }
    }
    ...
    public final long[] get(final String path) throws IOException{
        if(isStarted){
            ...
            if(TransactionsStats.getInstance().isEnabled()){
                TransactionsStats.getInstance().pushResolvingCacheStats(
                        new TransactionsStats.ResolvingCacheStat(
                                TransactionsStats.ResolvingCacheStat.Op.GET, elapsed,
                                getRoundTrips(path)));
            }
            return result;
        }
    ...
    public final INodeIdentifier get(final long inodeId) throws IOException {
        if (isStarted) {
            ...
            if (TransactionsStats.getInstance().isEnabled()) {
                TransactionsStats.getInstance().pushResolvingCacheStats(
                        new TransactionsStats.ResolvingCacheStat(TransactionsStats.ResolvingCacheStat.Op.GET, elapsed, 1));
            }
            return result;
        }
        return null;
