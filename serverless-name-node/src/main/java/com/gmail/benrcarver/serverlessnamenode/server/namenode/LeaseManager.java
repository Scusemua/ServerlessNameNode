    ...
    Lease addLease(String holder, String src)
        ...
        if (lease == null) {
            lease = new Lease(holder,
                    com.gmail.benrcarver.serverlessnamenode.server.namenode.Lease.getHolderId(holder)
                    , now(), 0);
    ...
    Lease reassignLease(Lease lease, String src, String newHolder)
        ...
        if (newLease == null) {
            newLease = new Lease(newHolder,
                    com.gmail.benrcarver.serverlessnamenode.server.namenode.Lease.getHolderId(newHolder)
                    , now(), 0);
