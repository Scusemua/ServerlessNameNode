...
import com.gmail.benrcarver.serverlessnamenode.protocol.BlockTokenIdentifier.AccessMode;
...
public class BlockManager {
     ...
     * new active. This is to make sure that this ServerlessNameNode has been
    ...
    public BlockManager(final FSNameSystem namesystem, final Configuration conf)
            throws IOException {
        ...
        if (!isEnabled) {
            if (UserGroupInformation.isSecurityEnabled()) {
                String errMessage = "Security is enabled but block access tokens " +
                        ...
                        "when clients attempt to connect to a DataNode. Aborting ServerlessNameNode";
    ...
    */
    ...
    public LocatedBlocks createPhantomLocatedBlocks(INodeFile file, byte[] data,
        ...
        final BlockTokenIdentifier.AccessMode mode = needBlockToken ? BlockTokenIdentifier.AccessMode.READ : null;
...
//      DatanodeStorageInfo storage) throws IOException, StorageException {
                ...
                HopsTransactionalRequestHandler rescanPostponedMisreplicatedBlocksHandler = new HopsTransactionalRequestHandler(
                    ...
                    public void acquireLock(TransactionLocks locks) throws IOException {
                        ...
                        locks.add(
                                ...
                                .add(
                                        lf.getBlockRelated(LockFactory.BLK.RE, LockFactory.BLK.IV, LockFactory.BLK.CR, LockFactory.BLK.UR, LockFactory.BLK.ER));
                    }
            ...
            public Object performTask() throws IOException {
                ...
                for (BlockListAsLongs.BlockReportReplica brb : reportedBlocks) {
                    ...
                    if (storedBlock != null) {
                        ...
                        if (brb.getState() == HdfsServerConstants.ReplicaState.FINALIZED){
                            ...
                            new HopsTransactionalRequestHandler(HDFSOperationType.GET_VALID_BLK_LOCS) {
                                ...
                                public void acquireLock(TransactionLocks locks) throws IOException {
                                    ...
                                    locks.add(
                                            ...
                                            .add(lf.getSqlBatchedBlocksRelated(LockFactory.BLK.RE, LockFactory.BLK.IV));
                                }
    ...
    public void processIncrementalBlockReport(DatanodeRegistration nodeID,
                    ...
                    public void acquireLock(TransactionLocks locks) throws IOException {
                        ...
                        locks.add(
                                ...
                                .add(lf.getIndividualBlockLock(rdbi.getBlock().getBlockId(),
                                        inodeIdentifier))
                                .add(lf.getBlockRelated(LockFactory.BLK.RE, LockFactory.BLK.ER, LockFactory.BLK.CR, LockFactory.BLK.UR));
        ...
        try {
            ...
            if (blockLog.isDebugEnabled()) {
                blockLog.debug(
                        "*BLOCK* ServerlessNameNode.processIncrementalBlockReport: " + "from " + nodeID + " receiving: " + receiving[0] + ", "
            ...
            while (namesystem.isRunning()) {
                try {
                    if (namesystem.isLeader()) {
                        ...
                        updateState();
                        LOG.debug("FSNameSystem is not leader: will not run replication monitor");
                    }
        ...
        return (Integer) new HopsTransactionalRequestHandler(
            ...
            public void acquireLock(TransactionLocks locks) throws IOException {
                ...
                locks.add(lf.getIndividualINodeLock(INodeLockType.WRITE, inodeIdentifier, true))
                        ...
                        .add(lf.getBlockRelated(LockFactory.BLK.RE, LockFactory.BLK.ER, LockFactory.BLK.CR, LockFactory.BLK.PE, LockFactory.BLK.UR, LockFactory.BLK.UC));
            }
