...
public abstract class ByteArrayManager {
  ...
  static class CounterMap {
    /** @see Conf#countResetTimePeriodMs */
    private final long countResetTimePeriodMs;
  ...
  static class Impl extends ByteArrayManager {
    private final ByteArrayManager.Conf conf;
  
    private final ByteArrayManager.CounterMap counters;
    private final ByteArrayManager.ManagerMap managers;
  
    Impl(ByteArrayManager.Conf conf) {
      this.conf = conf;
     ...
     * 
     * The byte array allocated by this method must be returned for recycling
     * via the {@link ByteArrayManager.Impl#release(byte[])} method.
     *
     * @return a byte array with length larger than or equal to the given length.
     */
    @Override
    public byte[] newByteArray(final int arrayLength) throws InterruptedException {
      ...
      if (arrayLength == 0) {
        ...
        final ByteArrayManager.FixedLengthManager manager = managers.get(powerOfTwo, aboveThreshold);
     ...
     * 
     * The byte array may or may not be allocated
     * by the {@link ByteArrayManager.Impl#newByteArray(int)} method.
     * 
    ...
    public int release(final byte[] array) {
      ...
      if (array.length == 0) {
        ...
        final ByteArrayManager.FixedLengthManager manager = managers.get(array.length, false);
    ...
    ByteArrayManager.CounterMap getCounters() {
      return counters;
    }
  
    ByteArrayManager.ManagerMap getManagers() {
      return managers;
    }
  }
}
